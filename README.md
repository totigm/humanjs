...
- **AI agents** (Stagehand, Playwright MCP) that book flights, fill forms, or monitor accounts on behalf of users
...
**Planned — Stagehand adapter:** wrap a [Stagehand](https://github.com/browserbase/stagehand) agent so its existing `act()` / `agent()` / `extract()` calls run humanized, with no code changes. See the [roadmap](#roadmap).
...
- [ ] Stagehand adapter (`@humanjs/stagehand`)
