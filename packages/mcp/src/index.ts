...
- After typing into a search/filter — especially debounced — the list re-renders and elements MOVE. Call human_wait (e.g. 400ms) or human_wait_for_load before targeting a result. The cursor travels over a short window; if the layout shifts mid-travel the click lands on a stale spot.
...
- A click that navigates resolves when dispatched, NOT when the page loads. Before human_stop_recording or any action on the new page, call human_wait_for_load (or confirm with human_screenshot) — otherwise a recording cuts off before the destination renders.
