...
**HumanJS humanizes browser automation for AI agents (Stagehand, Playwright MCP), QA tests where real-pace timing exposes bugs, and demo/tutorial recordings.**
...
Verified available on npm at project start: `@humanjs/core`, `@humanjs/playwright`, `@humanjs/recorder`, `@humanjs/stagehand`, `@humanjs/mcp`, `@humanjs/generator`, `@humanjs/skill`, `@humanjs/recipes`. Unscoped `humanjs` is owned by a dormant 2022 package — irrelevant scope, doesn't block us.
...
- AI agent adapters: Stagehand, MCP, plus Claude/Cursor/Codex skill
